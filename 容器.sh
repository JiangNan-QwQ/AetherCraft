...
if [ -d "$UBUNTU_ROOT" ]; then
    ...
    echo "移动 $YUANWEI_ZHI/3.3.sh 到 $UBUNTU_ROOT"
    cp $YUANWEI_ZHI/3.3.sh $UBUNTU_ROOT
    echo "移动 $YUANWEI_ZHI/容器换源.sh 到 $UBUNTU_ROOT"
    cp $YUANWEI_ZHI/容器换源.sh $UBUNTU_ROOT 
fi
done
...
touch "$FIRST_LOGIN"  # 创建标记文件
    echo "换源。。。"
    sleep 1
   proot-distro login ubuntu -- bash -c "bash 容器换源.sh"
   proot-distro login ubuntu
else
    echo "欢迎，即将进入"
    sleep 1
    proot-distro login ubuntu -- bash -c "bash 3.3.sh"
 proot-distro login ubuntu
fi
